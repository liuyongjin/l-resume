...
description: >-
  ...
  HTML page for this project. Use for pixel-perfect HTML prototypes before porting
  to Vue/shadcn-vue. Faithful image-to-HTML cloning via render -> compare -> refine.
  Trigger when user gives an image/mockup and asks to 复刻/还原/clone/recreate, or
  wants a standalone HTML preview before Nuxt integration.
---
...
## l-resume project note

After the HTML replica looks correct, port it into **Vue 3 + Nuxt + shadcn-vue +
Tailwind** using the `screenshot-to-code` skill. Place throwaway HTML under
`frontend-resume-nuxt/tmp/` (do not commit unless asked).

If `scripts/shot.mjs` is unavailable, compare visually via `npm run dev` +
browser preview instead of automated screenshots.

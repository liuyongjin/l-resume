...
function collectResumeText(data: unknown, parts: string[] = []): string {
...
/** 检测简历主体语言 */
export function detectResumeLanguage(data: ResumeData | Record<string, unknown>): ResumeLanguage {
...
export function getTranslateButtonLabel(target: ResumeLanguage): string {
  return target === 'en' ? '翻译为英文' : '翻译为中文'
}

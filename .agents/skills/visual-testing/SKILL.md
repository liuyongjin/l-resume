...
name: visual-testing
description: >-
  UI visual regression testing for the l-resume Nuxt frontend. Playwright
  toHaveScreenshot baselines, responsive viewport checks, diff review workflows,
  and agent-driven screenshot QA against http://localhost:3000. Use when user
  asks for visual test, screenshot diff, UI regression, pixel diff, update
  baselines, or to verify pages match a design after image-to-code work.
license: MIT
metadata:
  author: kindlmann
  version: "2.0"
...
<objective>
Catch visual regressions that functional tests miss. For this repo, focus on
resume builder pages: homepage, resume editor, workflow execution, templates.
</objective>

## l-resume project defaults

| Item | Value |
|------|-------|
| App root | `frontend-resume-nuxt/` |
| Dev server | `npm run dev` → `http://localhost:3000` |
| Key routes | `/`, `/resume`, `/editor/:id`, `/preview/:id`, `/templates`, `/workflow/execution`, `/workflow/executions`, `/workflow/designer`, `/login` |
| Mobile apps | `frontend-mobile-expo/`, `frontend-mobile-flutter/` |
| UI stack | Vue 3 + Nuxt 4 + shadcn-vue + Tailwind |
| Baselines dir | `frontend-resume-nuxt/tests/visual/` (create when adding Playwright) |
| Temp screenshots | `frontend-resume-nuxt/tmp/visual/` and `tmp/visual/mobile/` (gitignored) |

**Workflow pairing:** after `screenshot-to-code` or `screenshot-to-html` changes,
run visual checks on affected routes before marking done. For Expo/Flutter style QA,
also use the `mobile-visual-qa` and `expo-native-ui` project skills.

### Quick agent QA (no Playwright required)

When Playwright is not installed, still verify UI visually:

1. Start dev server: `cd frontend-resume-nuxt && npm run dev`
2. Open each critical route at desktop (1280×720) and mobile (375×667)
3. Capture screenshots to `frontend-resume-nuxt/tmp/visual/{route}-{viewport}.png`
4. Compare against design image or previous baseline side-by-side
5. Check: layout overflow, typography scale, shadcn component states, dark mode

Use Browser MCP / Playwright MCP `browser_navigate` + `browser_take_screenshot` when available.

### Playwright setup (when user wants automated visual tests)
...
Place visual specs under `frontend-resume-nuxt/tests/visual/*.visual.spec.ts` — separate from
removed E2E specs; only screenshot comparisons, not navigation smoke tests.
